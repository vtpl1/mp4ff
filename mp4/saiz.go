...
// Info implements Box.
func (s *SaizBox) Info(w io.Writer, specificBoxLevels string, indent string, _ string) error {
...
func DecodeSaizSR(hdr BoxHeader, _ uint64, sr bits.SliceReader) (Box, error) {
	versionAndFlags := sr.ReadUint32()
	version := byte(versionAndFlags >> 24)
	b := SaizBox{ //nolint:exhaustruct
		Version: version,
	...
	if hdr.Size != b.expectedSize() {
		return nil, fmt.Errorf("saiz: expected size %d, got %d", b.expectedSize(), hdr.Size) //nolint:err113
	}
...
func NewSaizBox(capacity int) *SaizBox {
	return &SaizBox{ //nolint:exhaustruct
