...
func DecodeStyp(hdr BoxHeader, _ uint64, r io.Reader) (Box, error) {
...
func DecodeStypSR(hdr BoxHeader, _ uint64, sr bits.SliceReader) (Box, error) {
	b := StypBox{data: sr.ReadBytes(int(hdr.Size) - hdr.Hdrlen)} //nolint:gosec
...
func (b *StypBox) Size() uint64 {
	return uint64(boxHeaderSize + len(b.data)) //nolint:gosec
}
...
func (b *StypBox) Info(w io.Writer, _ string, indent string, _ string) error {
