...
var (
	ErrEndOfFileNotSupported                  = errors.New("size 0, meaning to end of file, not supported")
	ErrBoxHeaderSizeExceedsBoxSize            = errors.New("box header size exceeds box size")
	ErrBoxSizeIsTooBigForNormal4ByteSizeField = errors.New("box size is too big for normal 4-byte size field")
)
...
func EncodeHeader(b Box, w io.Writer) error {
	...
	if boxSize >= 1<<32 {
		return ErrBoxSizeIsTooBigForNormal4ByteSizeField
	}
	buf := make([]byte, boxHeaderSize)
	binary.BigEndian.PutUint32(buf, uint32(boxSize)) //nolint:gosec
...
func EncodeHeaderSW(b Box, sw bits.SliceWriter) error {
	...
	sw.WriteUint32(uint32(boxSize)) //nolint:gosec
	sw.WriteString(boxType, false)
	return nil
}

// EncodeHeaderWithSizeSW - encode a box header to a writer and allow for largeSize.
...
func DecodeBox(startPos uint64, r io.Reader) (Box, error) { //nolint:ireturn
	var b Box
...
func DecodeBoxLazyMdat(startPos uint64, r io.ReadSeeker) (Box, error) {
	...
	remainingLength := int64(h.Size) - int64(h.Hdrlen) //nolint:gosec
...
func readBoxBody(r io.Reader, h BoxHeader) ([]byte, error) {
	hdrLen := uint64(h.Hdrlen) //nolint:gosec
	if hdrLen == h.Size {
		return nil, nil
	}
	bodyLen := h.Size - hdrLen
	body, err := io.ReadAll(io.LimitReader(r, int64(bodyLen))) //nolint:gosec
	if err != nil {
		return nil, err
	}
	if len(body) != int(bodyLen) { //nolint:gosec
		return nil, fmt.Errorf("read box body length %d does not match expected length %d", len(body), bodyLen) //nolint:err113
	}
	return body, nil
}
...
const (
	boxHeaderSize = 8          // boxHeaderSize - standard size + name header
