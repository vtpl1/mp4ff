...
var (
	ErrUseDecodeEsDescriptorInstead        = errors.New("use DecodeESDescriptor instead")
	ErrExpectedDecoderConfigDescriptor     = errors.New("expected DecoderConfigDescriptor")
	ErrReadTooFarInEsDescriptor            = errors.New("read too far in ESDescriptor")
	ErrMissingDecoderConfigDescriptor      = errors.New("missing DecoderConfigDescriptor")
	ErrReadTooFarInDecoderConfigDescriptor = errors.New("read too far in DecoderConfigDescriptor")
)
...
func DecodeDescriptor(sr bits.SliceReader, maxNrBytes int) (Descriptor, error) {
	if maxNrBytes < 2 {
		return nil, fmt.Errorf("descriptor size %d too small", maxNrBytes) //nolint:err113
	}
	tag := sr.ReadUint8()
	if sr.AccError() != nil {
	...
	switch tag {
	case ESDescrTag:
		return nil, ErrUseDecodeEsDescriptorInstead
	case DecoderConfigDescrTag:
...
func DecodeESDescriptor(sr bits.SliceReader, _ uint32) (ESDescriptor, error) {
	ed := ESDescriptor{} //nolint:exhaustruct
	tag := sr.ReadUint8()
	if tag != ESDescrTag {
		return ed, fmt.Errorf("got tag %d instead of ESDescriptorTag %d", tag, ESDescrTag) //nolint:err113
	}
	...
	currPos := sr.GetPos()
	nrBytesLeft := int(size) - (currPos - dataStart) //nolint:gosec
	...
	if !ok {
		return ed, ErrExpectedDecoderConfigDescriptor
	}
	currPos = sr.GetPos()
	nrBytesLeft = int(size) - (currPos - dataStart) //nolint:gosec
	desc, err = DecodeDescriptor(sr, nrBytesLeft)
	...
	for {
		...
		if nrBytesLeft < 0 {
			return ed, ErrReadTooFarInEsDescriptor
		}
		desc, err := DecodeDescriptor(sr, nrBytesLeft)
		if err != nil {
			sr.SetPos(currPos)
	...
	if size != ed.Size() {
		return ed, fmt.Errorf("read size %d differs from calculated size %d", size, ed.Size()) //nolint:err113
	}
...
func (e *ESDescriptor) EncodeSW(sw bits.SliceWriter) error {
	...
	if e.DecConfigDescriptor == nil {
		return ErrMissingDecoderConfigDescriptor
	}
...
func exceedsMaxNrBytes(sizeFieldSizeMinus1 byte, size uint64, maxNrBytes int) bool {
	return 1+uint64(sizeFieldSizeMinus1)+1+size > uint64(maxNrBytes) //nolint:gosec
}

func DecodeDecoderConfigDescriptor(tag byte, sr bits.SliceReader, maxNrBytes int) (Descriptor, error) {
	dd := DecoderConfigDescriptor{} //nolint:exhaustruct
	if tag != DecoderConfigDescrTag {
		return nil, fmt.Errorf("got tag %d instead of DecoderConfigDescrTag %d", tag, DecoderConfigDescrTag) //nolint:err113
	}
	sizeFieldSizeMinus1, size, err := readSizeSize(sr)
	if err != nil {
		return nil, err
	...
	if exceedsMaxNrBytes(sizeFieldSizeMinus1, size, maxNrBytes) {
		return nil, fmt.Errorf("DecoderConfigDescriptor size %d exceeds maxNrBytes %d", size, maxNrBytes) //nolint:err113
	}
	dataStart := sr.GetPos()
	...
	for {
		...
		if nrBytesLeft < 0 {
			return nil, ErrReadTooFarInDecoderConfigDescriptor
		}
		desc, err := DecodeDescriptor(sr, nrBytesLeft)
		if err != nil {
			sr.SetPos(currPos)
...
func DecodeDecSpecificInfoDescriptor(tag byte, sr bits.SliceReader, maxNrBytes int) (Descriptor, error) {
	dd := DecSpecificInfoDescriptor{} //nolint:exhaustruct
	if tag != DecSpecificInfoTag {
		return nil, fmt.Errorf("got tag %d instead of DecSpecificInfoTag %d", tag, DecSpecificInfoTag) //nolint:err113
	}
	...
	if exceedsMaxNrBytes(sizeFieldSizeMinus1, size, maxNrBytes) {
		return nil, fmt.Errorf("DecSpecificInfoDescriptor size %d exceeds maxNrBytes %d", size, maxNrBytes) //nolint:err113
	}
	dd.sizeFieldSizeMinus1 = sizeFieldSizeMinus1

	dataStart := sr.GetPos()
	dd.DecConfig = sr.ReadBytes(int(size))             //nolint:gosec
	bytesLeft := int(size) - (sr.GetPos() - dataStart) //nolint:gosec
	if bytesLeft > 0 {
		return nil, fmt.Errorf("DecSpecificInfoDescriptor has %d bytes left", bytesLeft) //nolint:err113
	}
...
func (d *DecSpecificInfoDescriptor) Info(w io.Writer, _ string, indent string, _ string) error {
	bd := newInfoDumper(w, indent, d, infoVersionDescriptor, 0)
...
func DecodeSLConfigDescriptor(tag byte, sr bits.SliceReader, maxNrBytes int) (Descriptor, error) {
	d := SLConfigDescriptor{} //nolint:exhaustruct
	if tag != SLConfigDescrTag {
		return nil, fmt.Errorf("got tag %d instead of SLConfigDescrTag %d", tag, SLConfigDescrTag) //nolint:err113
	}
	sizeFieldSizeMinus1, size, err := readSizeSize(sr)
	if err != nil {
		return nil, err
	...
	if exceedsMaxNrBytes(sizeFieldSizeMinus1, size, maxNrBytes) {
		return nil, fmt.Errorf("DecodeSLConfigDescriptor size %d exceeds maxNrBytes %d", size, maxNrBytes) //nolint:err113
	}
	d.sizeFieldSizeMinus1 = sizeFieldSizeMinus1

	d.ConfigValue = sr.ReadUint8()
	if size > 1 {
		d.MoreData = sr.ReadBytes(int(size - 1)) //nolint:gosec
	}
...
func (d *SLConfigDescriptor) Size() uint64 {
	return uint64(1 + len(d.MoreData)) //nolint:gosec
}
...
func (d *SLConfigDescriptor) Info(w io.Writer, specificLevels string, indent string, _ string) error {
	bd := newInfoDumper(w, indent, d, infoVersionDescriptor, 0)
	level := getInfoLevel(d, specificLevels)
	if level > 0 {
...
func DecodeRawDescriptor(tag byte, sr bits.SliceReader, maxNrBytes int) (Descriptor, error) {
	...
	if exceedsMaxNrBytes(sizeFieldSizeMinus1, size, maxNrBytes) {
		return nil, fmt.Errorf("DecRawDescriptor size %d exceeds maxNrBytes %d", size, maxNrBytes) //nolint:err113
	}
	//nolint:exhaustruct
	d := RawDescriptor{
		tag:                 tag,
	...
	d.data = sr.ReadBytes(int(size)) //nolint:gosec
	return &d, sr.AccError()
}
...
func (r *RawDescriptor) Info(w io.Writer, specificLevels string, indent string, _ string) error {
...
func CreateESDescriptor(decConfig []byte) ESDescriptor {
	...
	e := ESDescriptor{
		EsID: 0x01,
		DecConfigDescriptor: &DecoderConfigDescriptor{ //nolint:exhaustruct
			ObjectType: 0x40, // Audio ISO/IEC 14496-3,
			StreamType: 0x15, // 0x5 << 2 + 0x01 (audioType + upstreamFlag + reserved)
			DecSpecificInfo: &DecSpecificInfoDescriptor{ //nolint:exhaustruct
				DecConfig: decConfig,
			},
		},
		SLConfigDescriptor: &SLConfigDescriptor{
...
func readSizeSize(sr bits.SliceReader) (sizeFieldSizeMinus1 byte, size uint64, err error) { //nolint:nonamedreturns
	tmp := sr.ReadUint8()
...
func writeDescriptorSize(sw bits.SliceWriter, size uint64, sizeFieldSizeMinus1 byte) {
	for pos := int(sizeFieldSizeMinus1); pos >= 0; pos-- {
		value := byte(size>>uint32(7*pos)) & 0x7f //nolint:gosec
		if pos > 0 {
			value |= 0x80
		}
