...
func DecodeSidxSR(hdr BoxHeader, startPos uint64, sr bits.SliceReader) (Box, error) {
	...
	for range refCount {
		ref := SidxRef{}
		work := sr.ReadUint32()
		ref.ReferenceType = uint8(work >> 31) //nolint:gosec
		ref.ReferencedSize = work & 0x7fffffff
		ref.SubSegmentDuration = sr.ReadUint32()
		work = sr.ReadUint32()
		ref.StartsWithSAP = uint8(work >> 31)    //nolint:gosec
		ref.SAPType = uint8((work >> 28) & 0x07) //nolint:gosec
...
func (b *SidxBox) Size() uint64 {
	...
	return uint64(boxHeaderSize + 4 + 20 + 8*int(b.Version) + len(b.SidxRefs)*12) //nolint:gosec
}
...
func (b *SidxBox) EncodeSW(sw bits.SliceWriter) error {
	...
	if b.Version == 0 {
		sw.WriteUint32(uint32(b.EarliestPresentationTime)) //nolint:gosec
		sw.WriteUint32(uint32(b.FirstOffset))              //nolint:gosec
	} else {
		sw.WriteUint64(b.EarliestPresentationTime)
		sw.WriteUint64(b.FirstOffset)
	}
	sw.WriteUint16(0)                       // Reserved
	sw.WriteUint16(uint16(len(b.SidxRefs))) //nolint:gosec
...
func (b *SidxBox) Info(w io.Writer, specificBoxLevels string, indent string, _ string) error {
