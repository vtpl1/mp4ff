...
var (
	ErrLazyMdatModeExpectsNonNilReadseekerToReadData   = errors.New("lazy mdat mode - expects non-nil readseeker to read data")
	ErrNormalMdatModeInvalidRangeProvided              = errors.New("normal mdat mode - invalid range provided")
	ErrExtractionOfRangeFromDataPartsNotYetImplemented = errors.New("extraction of range from dataParts not yet implemented")
)
...
func DecodeMdatLazily(hdr BoxHeader, startPos uint64) (Box, error) {
	largeSize := hdr.Hdrlen > boxHeaderSize
	decLazyDataSize := hdr.Size - uint64(hdr.Hdrlen) //nolint:gosec
...
func (m *MdatBox) DataLength() uint64 {
	...
	return uint64(dataLength) //nolint:gosec
}

// Info - write box-specific information.
func (m *MdatBox) Info(w io.Writer, _ string, indent string, _ string) error {
...
func (m *MdatBox) HeaderSize() uint64 {
	...
	return uint64(hSize) //nolint:gosec
}
...
func (m *MdatBox) ReadData(start, size int64, rs io.ReadSeeker) ([]byte, error) {
	...
	if m.LazyDataSize > 0 {
		if rs == nil {
			return nil, ErrLazyMdatModeExpectsNonNilReadseekerToReadData
		}

		_, err := rs.Seek(start, io.SeekStart)
		if err != nil {
			return nil, fmt.Errorf("lazy mdat mode - unable to seek to %d", start) //nolint:err113
		}
	...
	offsetInMdatData := uint64(start) - mdatPayloadStart  //nolint:gosec
	endIndexInMdatData := offsetInMdatData + uint64(size) //nolint:gosec

	// validate if indexes are valid to avoid panics.
	dataLen := m.DataLength()
	if offsetInMdatData >= dataLen || endIndexInMdatData >= dataLen {
		return nil, ErrNormalMdatModeInvalidRangeProvided
	}
	if len(m.DataParts) > 0 {
		return nil, ErrExtractionOfRangeFromDataPartsNotYetImplemented
	}
	return m.Data[offsetInMdatData : offsetInMdatData+uint64(size)], nil //nolint:gosec
}

// CopyData - copy data range from mdat to w.
// The ReadSeeker is used for lazily loaded mdat case.
func (m *MdatBox) CopyData(start, size int64, rs io.ReadSeeker, w io.Writer) (int64, error) {
	// The Mdat box was decoded lazily
	if m.LazyDataSize > 0 {
		if rs == nil {
			return 0, ErrLazyMdatModeExpectsNonNilReadseekerToReadData
		}

		_, err := rs.Seek(start, io.SeekStart)
		if err != nil {
			return 0, fmt.Errorf("lazy mdat mode - unable to seek to %d", start) //nolint:err113
		}
	...
	dataLen := m.DataLength()
	if offsetInMdatData >= dataLen || endIndexInMdatData >= dataLen {
		return 0, ErrNormalMdatModeInvalidRangeProvided
	}
	if len(m.DataParts) > 0 {
		return 0, ErrExtractionOfRangeFromDataPartsNotYetImplemented
	}
	n, err := w.Write(m.Data[offsetInMdatData : offsetInMdatData+uint64(size)]) //nolint:gosec
	return int64(n), err
}
