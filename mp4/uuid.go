...
var (
	uuidTfxd     = MustCreateUUID(UUIDTfxd)     //nolint:gochecknoglobals
	uuidTfrf     = MustCreateUUID(UUIDTfrf)     //nolint:gochecknoglobals
	uuidPiffSenc = MustCreateUUID(UUIDPiffSenc) //nolint:gochecknoglobals
)
...
func (b *UUIDBox) SetUUID(uuid string) (err error) { //nolint:nonamedreturns
...
func DecodeUUIDBoxSR(hdr BoxHeader, startPos uint64, sr bits.SliceReader) (Box, error) {
	...
	case UUIDPiffSenc:
		if hdr.Size < 16 {
			return nil, fmt.Errorf("uuid box size too small: %d < 16", hdr.Size) //nolint:err113
		}
		...
		b.Senc = box.(*SencBox) //nolint:forcetypeassert
	default:
		if hdr.Size < 8+16 {
			return nil, fmt.Errorf("uuid box size too small: %d < 24", hdr.Size) //nolint:err113
		}
		b.UnknownPayload = sr.ReadBytes(int(hdr.Size) - 8 - 16) //nolint:gosec
	}
...
func decodeTfxd(s bits.SliceReader) (*TfxdData, error) { //nolint:unparam
...
func (t *TfxdData) encode(sw bits.SliceWriter) error {
	...
	if t.Version == 0 {
		sw.WriteUint32(uint32(t.FragmentAbsoluteTime))     //nolint:gosec
		sw.WriteUint32(uint32(t.FragmentAbsoluteDuration)) //nolint:gosec
	} else {
...
func decodeTfrf(s bits.SliceReader) (*TfrfData, error) { //nolint:unparam
	versionAndFlags := s.ReadUint32()
	version := byte(versionAndFlags >> 24)
	t := &TfrfData{ //nolint:exhaustruct
		Version:       version,
		Flags:         versionAndFlags & flagsMask,
...
func (t *TfrfData) encode(sw bits.SliceWriter) error {
	...
	if t.Version == 0 {
		for i := byte(0); i < t.FragmentCount; i++ {
			sw.WriteUint32(uint32(t.FragmentAbsoluteTimes[i]))     //nolint:gosec
			sw.WriteUint32(uint32(t.FragmentAbsoluteDurations[i])) //nolint:gosec
		}
	} else {
		for i := byte(0); i < t.FragmentCount; i++ {
...
func (b *UUIDBox) Info(w io.Writer, specificBoxLevels, indent, indentStep string) error {
	...
	if level > 0 {
		...
		case "tfrf":
			for i := range b.Tfrf.FragmentCount {
...
func UnpackKey(inKey string) (key []byte, err error) { //nolint:nonamedreturns
	...
	case 36:
		if inKey[8] != '-' || inKey[13] != '-' || inKey[18] != '-' || inKey[23] != '-' {
			return nil, fmt.Errorf("bad uuid format: %s", shorten(inKey)) //nolint:err113
		}
	...
	default:
		return nil, fmt.Errorf("cannot decode key %s", inKey) //nolint:err113
	}
	return key, nil
}
