...
func TestSPSParser1(t *testing.T) {
	...
	wanted := hevc.SPS{
		...
		ConformanceWindow: hevc.ConformanceWindow{
			LeftOffset:   0,
			RightOffset:  0,
			TopOffset:    0,
			BottomOffset: 2,
		...
		SubLayeringOrderingInfos: []hevc.SubLayerOrderingInfo{
			{
		...
		ShortTermRefPicSets: []hevc.ShortTermRPS{
			{
	...
	got, err := hevc.ParseSPSNALUnit(byteData)
	if err != nil {
...
func TestSPSParser3(t *testing.T) {
	...
	wantedHrd := hevc.HrdParameters{
		...
		SubLayerHrd: []hevc.SubLayerHrd{
			{
				NalHrdParameters: []hevc.SubLayerHrdParameters{
					{
...
func TestParseSPSWithNonZeroNumDeltaPocs(t *testing.T) {
	...
	hevcd, err := hevc.DecodeHEVCDecConfRec(data)
	if err != nil {
		t.Error(err)
	}
...
func TestSPSandPPS(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			pps, err := hevc.ParsePPSNALUnit(ppsBytes, spsMap)
			if err != nil {
