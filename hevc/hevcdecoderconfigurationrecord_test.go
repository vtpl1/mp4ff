...
func TestCreateDecConfRec(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		dcr, err := hevc.CreateHEVCDecConfRec(vpsNalus, spsNalus, ppsNalus,
			tc.complete, tc.complete, tc.complete, tc.includePS)
		seiArray := hevc.NewNaluArray(tc.complete, hevc.NALU_SEI_PREFIX, seiNalus)
		dcr.AddNaluArrays([]hevc.NaluArray{seiArray})
		if tc.errorMsg != "" {
...
func TestDecodeConfRec(t *testing.T) {
	...
	hdcr, err := hevc.DecodeHEVCDecConfRec(data)
	if err != nil {
		t.Error(err)
	}
	for _, na := range hdcr.NaluArrays {
		switch na.NaluType() {
		case hevc.NALU_VPS, hevc.NALU_PPS:
		case hevc.NALU_SPS:
