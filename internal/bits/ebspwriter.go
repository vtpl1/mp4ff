...
func (w *EBSPWriter) WriteExpGolomb(nr uint) {
	...
	w.Write(1, int(prefixLen+1)) //nolint:gosec
	if prefixLen > 0 {
		w.Write(delta, int(prefixLen)) //nolint:gosec
	}
}
...
func (w *EBSPWriter) NrBitsInBuffer() uint {
	return uint(w.n) //nolint:gosec
}

// BitsInBuffer - n bits written in buffer byte, not written to underlying writer.
func (w *EBSPWriter) BitsInBuffer() (uint, uint) {
	return w.v, uint(w.n) //nolint:gosec
}
