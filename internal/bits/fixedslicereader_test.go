...
func TestFixedSliceReader(t *testing.T) {
	...
	t.Run("read bytes", func(t *testing.T) {
		...
		sr.SetPos(nrBytes + 2)
		wantedErrMsg := bits.ErrAttemptToSkipBytesBeyondSliceLen // fmt.Sprintf("attempt to set pos %d beyond slice len %d", nrBytes+2, nrBytes)
		if sr.AccError() != wantedErrMsg {
