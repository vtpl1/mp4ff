...
func TestAudioSpecificConfigEncodeDecode(t *testing.T) {
	testCases := []aac.AudioSpecificConfig{
		{
	...
	for _, asc := range testCases {
		...
		gotAsc, err := aac.DecodeAudioSpecificConfig(readBuf)
		if err != nil {
			t.Error(err)
		}
...
func TestVariousInputs(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run(c.desc, func(t *testing.T) {
			...
			_, err := aac.DecodeAudioSpecificConfig(readBuf)
