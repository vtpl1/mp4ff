...
func TestADTS(t *testing.T) {
	adtsHdrStart, err := aac.NewADTSHeader(48000, 2, 2, 412)
	if err != nil {
		t.Error(err)
	}
	...
	for _, tc := range testCases {
		gotHdr, gotOffset, gotErr := aac.DecodeADTSHeader(bytes.NewBuffer(tc.adtsBytes))
...
func TestAdtsHdrLengthWithCRC(t *testing.T) {
	...
	gotHdr, gotOffset, err := aac.DecodeADTSHeader(bytes.NewBuffer(data))
	if err != nil {
		t.Error(err)
	}
	if gotOffset != 0 {
...
func TestAdtsHeaderParsingWithDoubleFF(t *testing.T) {
	...
	_, gotOffset, err := aac.DecodeADTSHeader(bytes.NewBuffer(data))
	if err != nil {
		t.Error(err)
	}
	wantedOffset := 2
